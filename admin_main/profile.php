              ...
              <div class="tab-content pt-2">
                ...
                <div class="tab-pane fade pt-3" id="profile-change-password">
                  ...
                  <form action="" method="POST">
                    ...
                    <div class="text-center">
                      <button type="submit" name="update" class="btn btn-primary">เปลี่ยนรหัสผ่าน</button>
                    </div>
                  </form><!-- End Change Password Form -->
                  <?php
                  if (!isset($_POST['update'])) {}else {
                  $id_admin = $_POST['id_admin'];
                  $password = $_POST['password'];
                  $newpassword = $_POST['newpassword'];
                  $renewpassword = $_POST['renewpassword'];
                  include "connect.php";
                  $sql = "SELECT p_word FROM admin where p_word = '$password'";
                  $result = mysqli_query($conn, $sql);
                  $num = mysqli_num_rows($result);
                  if ($num == 0) {
                    echo "<br><div class= 'alert alert-danger' role='alert'>Old password incorrect";
                  }else if ($newpassword != $renewpassword) {
                    echo "<br><div class= 'alert alert-danger' role='alert'>Password is not same";
                  }else {
                    $sql = "UPDATE admin SET p_word = '$newpassword' where id_admin = '$id_admin'";
                    $query = mysqli_query($conn, $sql);
                    echo "<br><div class= 'alert alert-success' role='alert'>เปลี่ยนรหัสผ่านสำเร็จ";
                    $conn->close();
                    }
                  }
                  ?>
