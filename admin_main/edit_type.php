...
<body>
  ...
  <main id="main" class="main">
    ...
    <section class="section dashboard">
      <?php
          include "connect.php";
          $sql = "SELECT*FROM type_accom where id_type ='".$_GET['id_type']."'";
          $sqlquery = mysqli_query($conn, $sql);
          $sqlnumrow = mysqli_num_rows($sqlquery);
          $row = mysqli_fetch_array($sqlquery);
      ?>
      <div class="row">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">แก้ไขข้อมูลประเภทที่พัก</h5>
            ...
            <form class="row g-3" method="post">
              <div class="col-md-12">
                ...
                <input name="name_type" value="<?php echo $row['name_type']; ?>" type="text" class="form-control" id="inputName" placeholder="เช่น บ้านพัก">
              </div>
            ...
            include "connect.php";
            $check = "SELECT*FROM type_accom where name_type = '$name_type'";
            $result = mysqli_query($conn, $check) or die (mysqli_error());
            $num = mysqli_num_rows($result);
            if ($num > 0) {
              echo "ข้อมูลซ้ำ กรุณากรอกใหม่";
            }else {
            $sql = "UPDATE type_accom SET id_type = '$id_type', name_type = '$name_type' where id_type = '$id_type'";
